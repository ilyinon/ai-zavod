...
- Быстрый новый чат без обязательного проекта и названия.
- Независимая история, поиск, закрепление, архив и выбор модели/команды на чат.
- Подключение рабочей папки перед запуском файловой задачи.
- Очередь выполнения в одном проекте и изоляция событий по taskId.
- [Контракт, ограничения и проверки](CHAT_PROJECT_UX.md).
