...
Вопросы вроде «как написать сортировку на Go?» получают пример в чате без запуска
разработки. Вопрос «какая погода в Нижнем Новгороде?» направляется к внешним
источникам даже без слова «загугли»; объяснение погодного API остаётся прямым ответом.
Неоднозначный запрос сначала уточняет режим. При сетевом сбое модель
вызывается не более трёх раз в пределах 180 секунд; это не запускает исправление кода.
Для остановленных подготовительных этапов доступен «Повторить запрос к модели»:
успешные результаты сохраняются, изменившийся проект или этап с побочными эффектами
не переигрывается автоматически. [Контракт и ограничения](docs/INTENT_AND_LIFECYCLE_RELIABILITY_FIX.md).
