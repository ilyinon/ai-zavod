// Cynhyrchwyd y ffeil hon yn awtomatig. PEIDIWCH Â MODIWL
// This file is automatically generated. DO NOT EDIT
import {app} from '../models';
import {agentgroups} from '../models';
import {project} from '../models';
import {llm} from '../models';
import {webresearch} from '../models';

export function AddAgentFromLibrary(arg1:app.AddAgentFromLibraryInput):Promise<Array<agentgroups.Profile>>;

export function AddExistingProject(arg1:app.AddExistingProjectInput):Promise<project.Project>;

export function ApplyWorkflowChanges(arg1:app.ApplyWorkflowChangesInput):Promise<app.ChatState>;

export function ArchiveAgentGroup(arg1:app.ArchiveAgentGroupInput):Promise<Array<agentgroups.Group>>;

export function BindProjectAgentGroup(arg1:app.BindProjectAgentGroupInput):Promise<app.ProjectState>;

export function Bootstrap():Promise<app.BootstrapState>;

export function CheckModel(arg1:string):Promise<Array<llm.ModelConfig>>;

export function CreateAgentGroup(arg1:app.CreateAgentGroupInput):Promise<Array<agentgroups.Group>>;

export function CreateAgentGroupFromTemplate(arg1:app.CreateAgentGroupFromTemplateInput):Promise<Array<agentgroups.Group>>;

export function CreateProject(arg1:app.CreateProjectInput):Promise<project.Project>;

export function DeleteLifecycleStep(arg1:app.DeleteLifecycleStepInput):Promise<Array<agentgroups.LifecycleStep>>;

export function DeleteProject(arg1:app.DeleteProjectInput):Promise<app.BootstrapState>;

export function DuplicateAgentProfile(arg1:app.DuplicateAgentProfileInput):Promise<Array<agentgroups.Profile>>;

export function GetAgentSoul(arg1:string):Promise<app.AgentSoulDTO>;

export function ListAgentGroupTemplates():Promise<Array<app.AgentGroupTemplateDTO>>;

export function ListAgentGroups():Promise<Array<agentgroups.Group>>;

export function ListAgentLibrary():Promise<Array<app.AgentLibraryItemDTO>>;

export function ListAgentProfiles(arg1:string):Promise<Array<agentgroups.Profile>>;

export function ListLifecycleDefinitions(arg1:string):Promise<Array<agentgroups.LifecycleDefinition>>;

export function ListLifecycleSteps(arg1:string):Promise<Array<agentgroups.LifecycleStep>>;

export function ListProjects(arg1:string):Promise<Array<project.Project>>;

export function ReplaceAgentSoulFromLibrary(arg1:app.ReplaceAgentSoulFromLibraryInput):Promise<app.AgentSoulDTO>;

export function RollbackWorkflowChanges(arg1:app.RollbackWorkflowChangesInput):Promise<app.ChatState>;

export function RunReview(arg1:app.RunReviewInput):Promise<app.ChatState>;

export function RunTestCommand(arg1:app.RunTestCommandInput):Promise<app.ChatState>;

export function SaveAgentProfile(arg1:app.SaveAgentProfileInput):Promise<Array<agentgroups.Profile>>;

export function SaveAgentSoul(arg1:app.SaveAgentSoulInput):Promise<app.AgentSoulDTO>;

export function SaveLifecycleDefinition(arg1:app.SaveLifecycleDefinitionInput):Promise<Array<agentgroups.LifecycleDefinition>>;

export function SaveLifecycleStep(arg1:app.SaveLifecycleStepInput):Promise<Array<agentgroups.LifecycleStep>>;

export function SaveModelConfig(arg1:app.SaveModelConfigInput):Promise<Array<llm.ModelConfig>>;

export function SaveWebSettings(arg1:app.SaveWebSettingsInput):Promise<webresearch.Settings>;

export function SelectProject(arg1:string):Promise<app.ProjectState>;

export function SendMessage(arg1:app.SendMessageInput):Promise<app.ChatState>;

export function SetActiveModel(arg1:string):Promise<Array<llm.ModelConfig>>;

export function SetAgentProfileEnabled(arg1:app.SetAgentProfileEnabledInput):Promise<Array<agentgroups.Profile>>;

export function SubmitClarification(arg1:app.SubmitClarificationInput):Promise<app.ChatState>;

export function UpdateAgentGroup(arg1:app.UpdateAgentGroupInput):Promise<Array<agentgroups.Group>>;

export function UpdateProject(arg1:app.UpdateProjectInput):Promise<project.Project>;
